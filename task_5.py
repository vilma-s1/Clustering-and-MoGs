...
X_phoneme = np.zeros((np.sum(phoneme_id == p_id), 3))  # initialising array that stores data samples of a given phoneme
j = 0
for i in range(0, len(phoneme_id)):
    if phoneme_id[i] == p_id:  # checks if the phoneme is the required phoneme (i.e., 1 or 2)
        X_phoneme[j, :] = X_full[i, :]  # stores data samples of the given phoneme in X_phoneme
        j +=1  # counter for the rows in X_phoneme
...
for t in range(n_iter):
    ...
    for i in range(k):
        ...
        # add a regularisation term to the covariance matrix sigma to avoid singularity
        identity = np.identity(D)  # identity matrix for regularisation term
        noise = 0.1  # initialise noise hyper-parameter
        s[i, :, :] += noise*identity
